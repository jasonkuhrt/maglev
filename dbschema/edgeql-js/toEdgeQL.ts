...
import { Cardinality, ExpressionKind, OperatorKind, TypeKind, util } from 'gel/dist/reflection/index'
...
import type { $expr_Delete, $expr_Select, LimitExpression, OffsetExpression } from './select'
...
export function $toEdgeQL(this: any) {
  ...
  for (const [expr, refData] of seen) {
    ...
    if (refData.linkProps.length) {
      linkProps.set(
        expr,
        refData.linkProps.map((linkProp) => linkProp.__parent__.linkName.slice(1)),
      )
    }
...
function renderEdgeQL(
  ...
  if (expr.__kind__ === ExpressionKind.With) {
    ...
    if (
      ...
      throw new Error(
        `Invalid arguments to set constructor: ${
          exprs
    ...
    return `(${
      renderEdgeQL(expr.__expr__, ctx, false, true).slice(
        1,
        -1,
      )
    }${clause.join('')})`
  ...
  elements: ${elementsShapeQuery}
    ...
    switch (expr.__opkind__) {
      ...
      case OperatorKind.Ternary:
        if (operator === 'if_else') {
          return `(${renderEdgeQL(args[0]!, ctx)} IF ${
            renderEdgeQL(
              args[1]!,
              ctx,
            )
...
function shapeToEdgeQL(
  ...
  const lines: string[] = []
  const addLine = (line: string) => lines.push(`${keysOnly ? '' : '  '}${line}`)
  ...
  for (const key in shape) {
    ...
    const expectedCardinality = addCardinalityAnnotations
    ...
    addLine(
      `${expectedCardinality}${q(key, false)} ${operator} ${wrapAssertExists ? 'assert_exists(' : ''}${
        renderedExpr.includes('\n')
          ? `(\n${
            indent(
              renderedExpr[0] === '('
