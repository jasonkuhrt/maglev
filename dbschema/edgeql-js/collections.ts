...
export function array<
  ...
  Exprs extends orScalarLiteral<
    TypeSet<
      Expr extends TypeSet ? getPrimitiveBaseType<Expr['__element__']>
...
type PropertyNamesFromPointers<Pointers extends ObjectTypePointers> = {
  [
    k in keyof Pointers as Pointers[k] extends PropertyDesc ? Pointers[k]['computed'] extends true ? never
      : k
      : never
  ]: Pointers[k]
}
...
> extends infer Pointers ? Pointers extends ObjectTypePointers ? NamedTupleType<
      {
        [
          k in keyof Pointers as k extends 'id' ? never
            : k
        ]: Pointers[k]['target']
      }
    >
