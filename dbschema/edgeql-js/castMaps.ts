...
export interface ScalarAssignableByMap {
  ...
  'default::Theme': _$default.$Theme
  'default::ProjectStatus': _$default.$ProjectStatus
...
export type scalarAssignableBy<T extends $.ScalarType> = T extends
  $.ScalarType<infer N extends keyof ScalarAssignableByMap> ? ScalarAssignableByMap[N]
  : never
...
export type scalarCastableFrom<T extends $.ScalarType> = T extends
  $.ScalarType<infer N extends keyof ScalarCastableFromMap> ? ScalarCastableFromMap[N]
  : never

type getSharedParentScalar<A, B> = A extends $.ScalarType<infer AName>
  ? B extends $.ScalarType<infer BName> ? AName extends 'std::decimal' ? BName extends 'std::decimal' ? B
      : BName extends 'std::bigint' ? A
      : never
    : AName extends 'std::cal::relative_duration' ? BName extends 'std::cal::relative_duration' ? B
      : BName extends 'std::cal::date_duration' ? A
      : never
    : AName extends 'std::cal::local_datetime' ? BName extends 'std::cal::local_datetime' ? B
      : BName extends 'std::cal::local_date' ? A
      : never
    : AName extends 'std::cal::local_date' ? BName extends 'std::cal::local_datetime' ? B
      : BName extends 'std::cal::local_date' ? B
      : never
    : AName extends 'std::cal::date_duration' ? BName extends 'std::cal::relative_duration' ? B
      : BName extends 'std::cal::date_duration' ? B
      : never
    : AName extends 'std::bigint' ? BName extends 'std::decimal' ? B
      : BName extends 'std::bigint' ? B
      : never
    : AName extends BName ? A
    : never
  : never
  : never
...
function getSharedParentScalar<A extends $.ScalarType, B extends $.ScalarType>(a: A, b: B): A | B {
  ...
  if (a.__name__ === 'default::Theme') {
    if (b.__name__ === 'default::Theme') {
      return b
    }
    throw new Error(`Types are not castable: ${a.__name__}, ${b.__name__}`)
  }
  if (a.__name__ === 'default::ProjectStatus') {
    if (b.__name__ === 'default::ProjectStatus') {
      return b
    }
    throw new Error(`Types are not castable: ${a.__name__}, ${b.__name__}`)
  }
...
type getTsType<T extends $.BaseType> = T extends $.ScalarType ? T extends
    | _std.$decimal
    | _stdfts.$document
    | _std.$json
    | _stdpg.$date
    ...
    | _std.$uuid ? never
  : T['__tstype__']
  : T extends $.RangeType ? gel.Range<T['__element__']['__tstype__']>
  : T extends $.MultiRangeType ? gel.MultiRange<T['__element__']['__tstype__']>
  : never
export type orScalarLiteral<T extends $.TypeSet> =
  | T
  | ($.BaseTypeSet extends T ? scalarLiterals
    : $.Cardinality extends T['__cardinality__'] ? getTsType<T['__element__']>
    : $.computeTsTypeCard<getTsType<T['__element__']>, T['__cardinality__']>)
...
> = $.ScalarType<
  ...
  TsConstType
>
export type literalToScalarType<T extends any> = T extends number ? scalarWithConstType<_std.$number, T>
...
type literalToTypeSet<T extends any> = T extends $.TypeSet ? T
