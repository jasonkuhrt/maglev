...
import type { MultiRange, Range } from 'gel'
import type { Executor } from 'gel/dist/ifaces'
import type { Cardinality, ExpressionKind, typeutil } from 'gel/dist/reflection/index'
...
type $jsonDestructure<Set extends TypeSet> = Set extends TypeSet<ScalarType<'std::json'>> ? {
...
export type ExpressionMethods<Set extends TypeSet> = {
  ...
  assert_single(): assert_single<
    ObjectType extends Set['__element__'] ? ObjectType<string, ObjectTypePointers, null>
      : Set['__element__'],
...
export type omitBacklinks<T extends string | number | symbol> = T extends `<${string}` ? never
  : T extends string ? T
  : never

export type stripNonUpdateables<T extends ObjectTypePointers> = {
  [k in keyof T]: [T[k]['computed']] extends [true] ? never
    : [T[k]['readonly']] extends [true] ? never
...
> = [Element] extends [true] ? pointerToTsType<Pointer>
  : [Element] extends [false] ? never
  : [Element] extends [boolean] ? pointerToTsType<Pointer> | undefined
  : Element extends TypeSet ? setToTsType<TypeSet<Element['__element__'], Pointer['cardinality']>>
  : Pointer extends LinkDesc ? Element extends object ? computeTsTypeCard<
        typeutil.flatten<
          & ([ParentTypeName] extends [string] ? Element extends { name: true } ? { name: ParentTypeName }
            : unknown
            : unknown)
          & computeObjectShape<
            Pointer['target']['__pointers__'] & Pointer['properties'],
            Element,
            (typeof future)['strictTypeNames'] extends true ? Pointer['target']['__polyTypenames__']
              : string
          >
        >,
        Pointer['cardinality']
...
> = (typeof future)['polymorphismAsDiscriminatedUnions'] extends true ? computeObjectShapeNew<Pointers, Shape, TypeName>
...
> = typeutil.flatten<
  keyof Shape extends never ? { id: string }
    : typeutil.stripNever<
      {
        ...
        > ? [k] extends [keyof PolyType['__element__']['__pointers__']] ?
          ...
          : Shape[k] extends TypeSet
            ? [k] extends [keyof Pointers] ? Shape[k]['__cardinality__'] extends cardutil.assignable<
                Pointers[k]['cardinality']
              > ? setToTsType<Shape[k]>
              : never
            : setToTsType<Shape[k]>
          : [k] extends [keyof Pointers] ? shapeElementToTs<
              Pointers[k],
              Shape[k],
              k extends '__type__' ? TypeName : null
            >
  ...
  :
    & typeutil.flatten<
      typeutil.stripNever<
        {
          [
            k in keyof Shape as Shape[k] extends $expr_PolyShapeElement ? never
              : k
          ]: Shape[k] extends TypeSet
            ? [k] extends [keyof Pointers] ? Shape[k]['__cardinality__'] extends cardutil.assignable<
                Pointers[k]['cardinality']
    ...
    & ({
      [
        k in keyof Shape as Shape[k] extends $expr_PolyShapeElement ? k
          : never
      ]: Shape[k]
    } extends infer PolyEls ? keyof PolyEls extends never ? unknown
      : getPolyElTypes<PolyEls[keyof PolyEls]> extends infer PolyTypeName ?
          | (Exclude<TypeName, PolyTypeName> extends never ? never
            : {
      ...
      & {
        [
          k in keyof PolyEls as PolyEls[k] extends $expr_PolyShapeElement
            ? PolyTypeName extends PolyEls[k]['__polyType__']['__element__']['__polyTypenames__'] ? k
            : never
            : never
        ]: PolyEls[k] extends $expr_PolyShapeElement<
          infer PolyType,
          infer ShapeEl
        > ? [k] extends [keyof PolyType['__element__']['__pointers__']] ? shapeElementToTs<
              PolyType['__element__']['__pointers__'][k],
              ShapeEl,
          ...
          : never
          : never
      } extends infer Shape ? Shape extends unknown ? typeutil.stripNever<Shape>
      : never
    : never
    : never
>

type getPolyElTypes<El> = El extends $expr_PolyShapeElement ? El['__polyType__']['__element__']['__polyTypenames__']
  : never
  ...
  | ScalarType<'std::bytes'> ? {
    [index: number]: $expr_Operator<
      ...
      getPrimitiveBaseType<
        Set['__element__'] extends ArrayType<infer El> ? El
          : Set['__element__']
      >,
      Set['__cardinality__']
    > // >
...
type $tuplePathify<Set extends TypeSet> = Set['__element__'] extends TupleType
  ...
  : Set['__element__'] extends NamedTupleType ? addNamedTuplePaths<
...
export type baseTupleElementsToTupleType<T extends typeutil.tupleOf<TypeSet>> = {
  [k in keyof T]: T[k] extends TypeSet ? getPrimitiveBaseType<T[k]['__element__']>
    : never
}
export type tupleElementsToTupleType<T extends typeutil.tupleOf<TypeSet>> = baseTupleElementsToTupleType<T> extends
  BaseTypeTuple ? TupleType<baseTupleElementsToTupleType<T>>
  : never
...
export type tupleElementsToCardTuple<T> = baseTupleElementsToCardTuple<T> extends [Cardinality, ...Cardinality[]]
...
type addTuplePaths<Items extends BaseType[], ParentCard extends Cardinality> = {
  [k in indexKeys<keyof Items>]: Items[k] extends BaseType ? $expr_TuplePath<Items[k], ParentCard>
    : never
}
...
type inferNamedTupleCardinality<Shape extends NamedTupleLiteralShape> = [
  Cardinality.Many,
] extends [shapeCardinalities<Shape>] ? Cardinality.Many
  : [Cardinality.Empty] extends [shapeCardinalities<Shape>] ? Cardinality.Empty
  : [shapeCardinalities<Shape>] extends [Cardinality.AtMostOne] ? Cardinality.AtMostOne
...
> = {
  [k in keyof Shape]: Shape[k] extends BaseType ? $expr_TuplePath<Shape[k], ParentCard>
    : never
}
...
export type orLiteralValue<Set extends TypeSet> =
  | Set
  | (Set['__element__'] extends ObjectType ? never
...
type ArrayTypeToTsType<
  ...
  isParam extends boolean,
> = isParam extends true ? readonly BaseTypeToTsType<Type['__element__'], isParam>[]
  : BaseTypeToTsType<Type['__element__'], isParam>[]

export type BaseTypeToTsType<
  Type extends BaseType,
  isParam extends boolean = false,
> = BaseType extends Type ? unknown
  : Type extends ScalarType ? ScalarTypeToTsType<Type, isParam>
  : Type extends EnumType ? Type['__tstype__']
  : Type extends BaseArrayType ? ArrayTypeToTsType<Type, isParam>
  : Type extends RangeType ? Range<Type['__element__']['__tsconsttype__']>
  : Type extends MultiRangeType ? MultiRange<Type['__element__']['__tsconsttype__']>
  : Type extends TupleType ? TupleItemsToTsType<Type['__items__'], isParam>
  : Type extends NamedTupleType ? NamedTupleTypeToTsType<Type, isParam>
  : Type extends ObjectType ? computeObjectShape<
      Type['__pointers__'],
      Type['__shape__'],
      (typeof future)['strictTypeNames'] extends true ? Type['__polyTypenames__']
        : string
    >
  : never

export type setToTsType<Set> = Set extends $Shape<infer Element, infer Shape, infer Card>
  ? Shape extends object ? computeTsTypeCard<
      computeObjectShape<
        Element['__pointers__'],
        normaliseShape<Shape>,
        (typeof future)['strictTypeNames'] extends true ? Element['__polyTypenames__']
          : string // todo
      >,
      Card
    >
  : never
  : Set extends TypeSet ? computeTsType<Set['__element__'], Set['__cardinality__']>
  : never

export type computeTsTypeCard<T, C extends Cardinality> = Cardinality extends C ? unknown
  : C extends Cardinality.Empty ? null
  : C extends Cardinality.One ? T
  : C extends Cardinality.AtLeastOne ? [T, ...T[]]
  : C extends Cardinality.AtMostOne ? T | null
  : C extends Cardinality.Many ? T[]
