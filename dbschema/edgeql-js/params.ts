...
import { Cardinality, ExpressionKind, TypeKind } from 'gel/dist/reflection/index'
import { complexParamKinds } from './__spec__'
import { $expressionify } from './path'
import { runnableExpressionKinds } from './query'
import { select } from './select'
import type { BaseTypeToTsType, Expression, ParamType, setToTsType, TypeSet } from './typesystem'
...
type paramsToParamArgs<Params extends ParamsRecord> =
  & {
    [
      key in keyof Params as Params[key] extends ParamType ? key
        : never
    ]: Params[key] extends ParamType ? Readonly<BaseTypeToTsType<Params[key], true>>
      : never
  }
  & {
    [
      key in keyof Params as Params[key] extends $expr_OptionalParam ? key
        : never
    ]?: Params[key] extends $expr_OptionalParam ? Readonly<BaseTypeToTsType<Params[key]['__type__'], true> | null>
      : never
  }
...
> = Expression<{
  ...
  __element__: Type
  __cardinality__: Optional extends true ? Cardinality.AtMostOne
    : Cardinality.One
  __name__: Name
...
type paramsToParamExprs<Params extends ParamsRecord> = {
  [key in keyof Params]: Params[key] extends $expr_OptionalParam ? $expr_Param<key, Params[key]['__type__'], true>
    : Params[key] extends ParamType ? $expr_Param<key, Params[key], false>
    : never
}
