...
import { Cardinality, type introspect, TypeKind } from 'gel/dist/reflection/index'
...
export function $resolveOverload(
  ...
  throw new Error(
    `No function overload found for ${
      ...
      [...positionalArgs, ...Object.values(namedArgs ?? {})]
        .filter(Boolean)
        .map(
...
function _tryOverload(
  ...
  if (funcName === 'if_else') {
    ...
    cardinality = funcDef.returnTypemod === 'SetOfType'
      ? Cardinality.Many
  ...
  if (needsAnytypeReplacement) {
    ...
    positionalArgs = positionalArgs.map((arg) => (arg as any) === ANYTYPE_ARG ? cast(returnAnytype!, null) : arg)
  }
