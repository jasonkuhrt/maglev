...
> = TypeSet<
  ChildKey extends 'name' ? Root extends { [typenameSymbol]: string } ? ScalarType<
        'std::str',
        (typeof future)['strictTypeNames'] extends true ? Root[typeof typenameSymbol]
          : string,
...
> // Parent extends PathParent | null = null
 = Root extends ObjectTypeSet ? ObjectTypeSet extends Root ? unknown // Root is literally ObjectTypeSet
...
export type pathifyPointers<
  Root extends ObjectTypeSet,
> // Parent extends PathParent | null = null
 = ObjectTypePointers extends Root['__element__']['__pointers__'] ? unknown
  : {
    // & string required to avoid typeError on linkName
    [
      k in
        & keyof Root['__element__']['__pointers__']
        & string
    ]: Root['__element__']['__pointers__'][k] extends PropertyDesc ? $expr_PathLeaf<
      ...
      : Root['__element__']['__pointers__'][k] extends LinkDesc
        ? getChildOfObjectTypeSet<Root, k> extends ObjectTypeSet ? $expr_PathNode<
            getChildOfObjectTypeSet<Root, k>,
            { type: anonymizeObjectTypeSet<Root>; linkName: k },
            k extends '__type__' ? Root['__element__']['__polyTypenames__']
              : null
          >
        : unknown
      : unknown
...
> = string extends keyof Shape ? object
  : {
    [k in keyof Shape & string]: Shape[k] extends ObjectTypeSet ? $expr_PathNode<
        TypeSet<
          Shape[k]['__element__'],
          cardutil.multiplyCardinalities<
            Root['__cardinality__'],
      ...
      > // false
      : Shape[k] extends TypeSet ? $expr_PathLeaf<
          TypeSet<
            Shape[k]['__element__'],
            cardutil.multiplyCardinalities<
              Root['__cardinality__'],
...
type pathifyLinkProps<
  ...
  Parent extends PathParent | null = null,
> = {
  [k in keyof Props & string]: Props[k] extends PropertyDesc ? $expr_PathLeaf<
      TypeSet<
        Props[k]['target'],
        cardutil.multiplyCardinalities<
          Root['__cardinality__'],
...
export type getPropsShape<T extends ObjectType> = typeutil.flatten<
  typeutil.stripNever<
    {
      [k in keyof T['__pointers__']]: T['__pointers__'][k]['__kind__'] extends 'property' ? true
        : never
    }
  >
>
