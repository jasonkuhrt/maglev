...
import { Cardinality, ExpressionKind, TypeKind, type typeutil } from 'gel/dist/reflection/index'
...
export type pointerIsOptional<T extends PropertyDesc | LinkDesc> = T['cardinality'] extends
  | Cardinality.Many
  | Cardinality.Empty
  | Cardinality.AtMostOne ? true
  : false
...
export type RawInsertShape<El extends ObjectType> =
  ...
  ObjectType extends El ? never
    : typeutil.stripNever<
      stripNonInsertables<stripBacklinks<El['__pointers__']>>
    > extends infer Shape ? Shape extends ObjectTypePointers ?
          & typeutil.addQuestionMarks<
            {
              [k in keyof Shape]:
                | pointerToAssignmentExpression<Shape[k]>
                | (pointerIsOptional<Shape[k]> extends true ? undefined | null
                  : never)
...
> = Expression<{
  ...
  __element__: Conflict['else'] extends TypeSet
    ? Conflict['else']['__element__']['__name__'] extends El['__name__'] ? El
    : $Object
    : El
  __cardinality__: Conflict['else'] extends TypeSet ? Conflict['else']['__cardinality__']
...
export function $normaliseInsertShape(
  ...
  const _shape: [string, any][] = shape.__element__?.__kind__ === TypeKind.namedtuple
