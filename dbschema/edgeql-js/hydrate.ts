...
import type { BaseType, LinkDesc, ObjectType, ObjectTypePointers, PropertyDesc, TupleType } from './typesystem'

import { TypeKind, util } from 'gel/dist/reflection/index'
  ...
  if (type.kind === 'object') {
    ...
    if (type.tuple_elements[0]!.name === '0') {
      ...
      util.defineGetter(obj, '__items__', () => {
        return type.tuple_elements.map((el) => makeType(spec, el.target_id, literal, anytype)) as any
      })
      util.defineGetter(obj, '__name__', () => {
        return `tuple<${
          obj.__items__
...
> = A extends ObjectType ? B extends ObjectType ? ObjectType<
      `${A['__name__']} UNION ${B['__name__']}`,
      mergeObjectShapes<A['__pointers__'], B['__pointers__']>,
      null
    >
  : A
  : B extends ObjectType ? B
  : undefined
