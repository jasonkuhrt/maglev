...
import { ConfigMemory, DateDuration, Duration, LocalDate, LocalDateTime, LocalTime, RelativeDuration } from 'gel'
import type { $bool, $number } from './modules/std'

import { Cardinality, ExpressionKind, OperatorKind, TypeKind } from 'gel/dist/reflection/index'
...
import { type literalToScalarType, literalToTypeSet, type scalarLiterals } from './castMaps'
...
export type exclusivesToFilterSingle<E extends ExclusiveTuple> = ExclusiveTuple extends E ? never
  : E extends [] ? never
  : {
    [j in keyof E]: {
...
export interface SelectModifierMethods<Root extends TypeSet> {
  filter<Filter extends SelectFilterExpression>(
    filter:
      | Filter
      | ((
        scope: Root extends ObjectTypeSet ? $scopify<Root['__element__']>
          : stripSet<Root>,
      ) => Filter),
  ): this
  order_by(
...
> = Modifiers['limit'] extends number | LimitExpression ? cardutil.overrideLowerBound<Card, 'Zero'>
  : Modifiers['offset'] extends number | OffsetExpression ? cardutil.overrideLowerBound<Card, 'Zero'>
  : Card
...
export type ComputeSelectCardinality<
  ...
  Modifiers extends UnknownSelectModifiers,
> = InferOffsetLimitCardinality<
  undefined extends Modifiers['filter_single'] ? Expr['__cardinality__']
...
export function $handleModifiers(
  ...
  if (modifiers.offset) {
    mods.offset = typeof modifiers.offset === 'number'
  ...
  if (modifiers.limit) {
    ...
    if (typeof modifiers.limit === 'number') {
      ...
      const type = (modifiers.limit.__element__ as any).__casttype__
...
type linkDescToShape<L extends LinkDesc> =
...
> =
  & Partial<
    {
      [k in keyof Pointers]: Pointers[k] extends PropertyDesc ? propDescToSelectElement<Pointers[k]>
        : Pointers[k] extends LinkDesc ? linkDescToSelectElement<Pointers[k]>
        : any
    }
  >
  & { [k: string]: unknown }
...
export type normaliseElement<El> = El extends boolean ? El
  : El extends TypeSet ? stripSet<El>
  : El extends (...scope: any[]) => any ? normaliseShape<ReturnType<El>>
  : El extends object ? normaliseShape<stripSet<El>>
  : stripSet<El>
...
function $shape<
  ...
  Scope extends
    ...
    & $linkPropify<
      {
        [k in keyof Expr]: k extends '__cardinality__' ? Cardinality.One
          : Expr[k]
      }
    >,
>(
...
export function select<Shape extends { [key: string]: TypeSet }>(
  ...
  __element__: ObjectType<
    `std::FreeObject`,
    {
      [k in keyof Shape]: Shape[k]['__element__'] extends ObjectType ? LinkDesc<
          Shape[k]['__element__'],
...
export function select(...args: any[]) {
  ...
  const exprPair: [TypeSet, (scope: any) => any] = typeof args[0].__element__ !== 'undefined'
    ? (args as any)
    : [FreeObject, () => args[0]]
...
export function resolveShapeElement(
  ...
  const isSubshape = typeof value === 'object' && typeof (value as any).__kind__ === 'undefined'
    ...
    if (!linkProp) {
      throw new Error(
        (scope as any).__parent__
          ? `link property '${key}' does not exist on link ${(scope as any).__parent__.linkName}`
